"""
Test different block sizes (7, 14, 21 days) to find optimal MAPE.
Usage: python3 optimize_block_size.py --model ensemble
"""
...
def test_block_sizes(model_preference: str = "ensemble", as_of_date: str = None):
    """Test different block sizes (7, 14, 21 days) with 60-day training window."""
    ...
    print(f"{'='*60}")
    print(f"Block Size Optimization ({model_preference}, 60-day train)")
    print(f"{'='*60}\n")
    
    results = []
    train_days = 60
    
    for block_days in [7, 14, 21]:
        print(f"\n--- Block Size: {block_days} days ---")
        ...
        results.append({"block_days": block_days, "mape": mape, "points": len(forecast_points)})
    
    # Summary
    print(f"\n{'='*60}")
    print("SUMMARY")
    print(f"{'='*60}")
    print(f"{'Block Days':<12} {'MAPE':<10} {'Points':<10}")
    print("-" * 35)
    for r in results:
        print(f"{r['block_days']:<12} {r['mape']:<10.2f} {r['points']:<10}")
    
    best = min(results, key=lambda x: x["mape"])
    print(f"\nBest: {best['block_days']} days with MAPE = {best['mape']}%")
    print(f"{'='*60}\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optimize block size")
    parser.add_argument("--model", type=str, default="ensemble",
                        help="Model: prophet, arimax, xgboost, ensemble")
    parser.add_argument("--as-of-date", type=str, default=None,
                        help="Filter data up to date (YYYY-MM-DD)")
    
    args = parser.parse_args()
    
    test_block_sizes(args.model, args.as_of_date)
